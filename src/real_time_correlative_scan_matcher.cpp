...
SearchParameters::SearchParameters(const double linear_search_window,
  ...
  for (const sensor::RangePoint& point : point_cloud) {
    const float range = point.position.head<2>().norm();
    max_scan_range = std::max(range, max_scan_range);
  }
  const double kSafetyMargin = 1. - 1e-3;
  angular_perturbation_step_size =
      kSafetyMargin * std::acos(1. -std::pow(resolution,2) /
                                         (2. * std::pow(max_scan_range,2)));
...
std::vector<sensor::PointCloud> GenerateRotatedScans(
  ...
  for (int scan_index = 0; scan_index < search_parameters.num_scans;
	   ...
	   rotated_scans.push_back(sensor::TransformPointCloud(point_cloud,Eigen::AngleAxisf(delta_theta, Eigen::Vector3f::UnitZ()).toRotationMatrix()));
  }
...
std::vector<DiscreteScan2D> DiscretizeScans(
  ...
  for (const sensor::PointCloud& scan : scans) {
    ...
    for (const sensor::RangePoint& point : scan) {
